...
def Evaluation(y_true,y_pred,y_pred_prob):
    ...
    aupr=average_precision_score(y_true, prob[:,1])

    eval_indicator['recall'] = recall
    eval_indicator['precision'] = precision
    eval_indicator['aupr']=aupr
    eval_indicator['auc']=auc
    eval_indicator['f1']=f1
...
def train(epoch):
    ...
    if epoch % 20 == 0:
        print("~~~~>Train acc: {:.4f} | f1:{:.4f} | epoch:{}".format(correct_num/data_num,eval_indicator['f1'],epoch))
...
def test(epoch):
    ...
    eval_indicator=Evaluation(target_all.cpu(),predicted_all.cpu(),evidence_all.cpu())
    if epoch % 100 == 0:
        print("====>TEST acc: {:.4f} | f1:{:.4f}".format(correct_num/data_num,eval_indicator['f1']))
...
if __name__ == "__main__":
    ...
    for repeat in range(10):
        for train_index, test_index in skf.split(data1, Y.astype('int')):
            ...
            costte=[]
            accte,f1_te,auc_te,recall_te,precision_te,aupr_te= [],[],[],[],[],[]
            ...
            for epoch in range(1, epochs + 1):
                ...
                f1_te.append(te_indicators['f1'])
                auc_te.append(te_indicators['auc'])
                recall_te.append(te_indicators['recall'])
                precision_te.append(te_indicators['precision'])
    ...
    f1_te_all=np.array(f1_te_all)
    auc_te_all=np.array(auc_te_all)
    ...
    f1_te_mean=sum(f1_te_all) / f1_te_all.shape[0]
    f1_te_std=np.std(f1_te_all,axis=0)
    
    auc_te_mean=sum(auc_te_all) / auc_te_all.shape[0]
    auc_te_std=np.std(auc_te_all,axis=0)
    ...
    print('f1:', round(max(f1_te_mean),4))
    print('auc:', round(max(auc_te_mean),4))
    ...
    f1_te_std_index=np.where(f1_te_mean==max(f1_te_mean))[0]
    f1_te_std=np.min(f1_te_std[f1_te_std_index])
    
    auc_te_std_index=np.where(auc_te_mean==max(auc_te_mean))[0]
    auc_te_std=np.min(auc_te_std[auc_te_std_index])
    ...
    with open("MOEDL 10fold.txt",'a')as f:
        ...
        f.write("F1: ({:.4f},{:.4f})".format(max(f1_te_mean),f1_te_std)+" ")
        f.write("auc: ({:.4f},{:.4f})".format(max(auc_te_mean),auc_te_std)+" ")
